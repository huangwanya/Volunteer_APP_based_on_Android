...
function listBlog(typeid){
    ...
    let sql = `select b.*,(select count(pid) from wct_replay r where r.pid=b.id) pinglun,u.img uimg from wct_blog b left join wct_user u on u.id=b.uid where b.uid in (${mfids})`;
    if(typeid){
...
function toMyZan(){
    ...
    ajaxCallback("listJ",{table:"zan",uid:userinfo.id},function (zanlist) {
        ...
        let sql = `select * from wct_blog where id in ('${bids}')`;
